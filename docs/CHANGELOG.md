...
- **席位声明（claims）：邮箱与「是否设了密码」现在会随 `export/devmon.json` 一起解码。** `personal_key_manager` 在
  TA1 许可格式上增加了可选的席位声明——每个席位可绑定一个由主密钥签名的邮箱，并可选择设置密码校验器。
  `SeatRecord` 新增 `email` / `hasPassword` / `claimsUpdatedAt`。这三个字段是声明的**投影而非证据**：只用于显示与
  反查，需要证明身份时仍应交给 pkm 校验证书，与 `revoked` 必须等签名校验通过才可信是同一个道理。声明与席位状态
  无关，因此在多来源合并（`stricterSeat`）里单独合并——邮箱取第一个非空值，`hasPassword` 任一为真即为真，
  `claimsUpdatedAt` 取最新——否则「较严格」的一方会把另一方的身份信息悄悄丢掉。**注意 pkm 对没有声明的席位也会
  导出 `email: null` 与 `hasPassword: false`**（只有 `claimsUpdatedAt` 整个省略），所以解码必须同时容忍「缺失」与
  「null」两种形式，`hasPassword` 因此用 `Bool?` 而不是 `Bool`。licence 的 `v` 与 bundle 的 `bundleVersion` 都没变，
  旧包与新包在这一版上都能正常解码。
- **签发弹窗新增邮箱与密码字段。** 邮箱在席位标识本身像邮箱时自动跟随（用户改动过之后就不再覆盖），提交前统一
  去空白 + 小写化，与 pkm 的规范化保持一致；本地**不做**邮箱格式校验，那是 pkm 的职责，另立一套规则容易和它对不上。
  密码是**可选**声明：留空即不设密码（本来就是合法席位），填了则必须通过两个确认框——只填一格也算不一致，
  否则「填了密码却忘了确认」会静默变成不设密码；短于 8 字符（与 pkm 的 `PASSWORD_MIN_LENGTH` 对齐）时签发按钮
  保持禁用，省掉一次注定失败的子进程往返。签发成功后弹窗会列出本次实际写入的声明，并明确说明重新签发声明
  **不会**让旧密钥失效——旧密钥照旧可用，只是不带声明，因此不会把交接中的席位锁在门外。
- **席位行显示身份，popover 许可标签页可检索。** 设置 → 许可的密钥树里，邮箱只在**与 `sub` 不同**时才显示
  （二者常常就是同一个值，无条件加一列只是噪音），`hasPassword` 为真时加一个 `pw` 胶囊标记。popover 的许可
  标签页新增检索框：输入片段按席位标识与邮箱做子串匹配；粘贴**完整邮箱**时先走一次大小写与前后空格无关的精确
  反查（`SeatRegistry.seat(forEmail(_:)`），命中即锁定那一个席位。反查读的是实时状态而列表是快照，两者不一致时
  自动退回子串匹配，不会把席位藏起来。
- **`scripts/check-public-safety.mjs` 增补密钥容器规则。** BLOCKERS 新增 `scrypt$…` 密码校验器与 `"pwdv"` 字段——
  pkm 只导出布尔值，真出现具体值就说明有人把校验器抄了出来。`TEXT_EXT` 增加 `.key` / `.pem` / `.asc` / `.p12` /
  `.jks` / `.keystore`：`.key` 此前只在 `PATH_BLOCKLIST` 里，而默认模式下黑名单文件会被**直接跳过**（既不读也不报），
  于是抄进来的 `<sub>.key`（内容就是一条真实 TA1 许可串）扫描结果是「干净」的；`PATH_BLOCKLIST` 同时补上
  `.pem` / `.asc` / `.jks` / `.keystore`，让这些容器在默认模式与 `--all` 下都被拦住。已用合成的 scrypt 值、
  `pwdv` 字段与 `.key` 文件验证四条规则都会触发。

### Changed

- **密码走 `--password-stdin`，不进 argv。** `ProcessRunner.run` 新增可选 `stdin` 参数（默认 nil，全部既有调用点
  不受影响）：建立 `Pipe`、写入后**立即关闭写端**，且这一步必须发生在等待退出**之前**——pkm 会一直读到 EOF，
  先等退出就会死锁到超时。写入前对该 fd 单独设置 `F_SETNOSIGPIPE`：子进程可能因参数错误先行退出，此时写入会
  收到 SIGPIPE 直接终止本进程，而全局改 signal handler 影响面太大。pkm 没有提供任何密码环境变量
  （`PKM_ROOT` 是整个 CLI 唯一的 `process.env` 读取），所以 stdin 是唯一不把密码暴露给同机 `ps` 与 shell 历史的
  通道。`KeyManager.run` 透传该参数，`issue` 只在密码确实存在时才追加 `--password-stdin`——空密码绝不能当成
  「要设密码」，而「没有密码」本身是合法状态。明文密码在提交后立即从弹窗状态清空，不依赖 SwiftUI 何时销毁视图，
  也不会进入日志或错误信息（`failureMessage` 会把子进程 stderr 提成用户可见文案，因此 argv 里更不能有密码）。
