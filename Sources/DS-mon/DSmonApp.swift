...
struct UsageBarChart: View {
        ...
        chartView
            .overlay(alignment: .topLeading) {
                if let bar = hoveredBar, hasData, let loc = hoverLocation {
                    let leftSide = loc.x < frameWidth * 0.45
                    tooltipContent(bar)
                        .fixedSize()
                        .offset(x: leftSide ? loc.x + 14 : loc.x - 180,
                                y: max(loc.y - 56, 4))
                }
            }
    }
    ...
    private func chartOverlay(proxy proxy: ChartProxy) -> some View {
    ...
    private func tooltipContent(_ bar: TokenBar) -> some View {
        ...
        let estCost = Double(bar.missTokens) / 1_000_000 * 1.0
            + Double(bar.hitTokens) / 1_000_000 * 0.02
            + Double(bar.outTokens) / 1_000_000 * 2.0
        VStack(alignment: .trailing, spacing: 2) {
            ...
            tooltipRow(Strings.costLabel, Strings.costShort(estCost))
        }
        .padding(.vertical, 6)
        .padding(.horizontal, 10)
