...
- 支持 **DeepSeek** / **Moonshot（Kimi）** / **OpenAI** / **Anthropic (Claude)** 等多提供商，弹窗中以标签页切换
- OpenAI / Anthropic 使用管理密钥（Admin Key）拉取本月费用与 token 用量
- 每 5 分钟自动刷新余额和可用模型
- 可自定义预警阈值（低于阈值红色闪烁）
...
- 顶部：应用名 + 状态徽章；其下为提供商标签页（样式与 Usage/License 等一致）
- 余额区：总余额、赠送余额、充值余额；费用型提供商显示本月费用与 token 用量
- 信息区：预警阈值、默认模型、可用模型列表、账户状态
- 用量区：日/周/月切换，柱状图/请求列表切换，柱状图支持鼠标悬停查看明细
- 底部操作栏（图标按钮，悬停提示）：刷新 / 导出用量 / 导出配置 / 导入配置 / 设置 / 退出
...
- DeepSeek / Moonshot API Key（OpenAI / Anthropic 可选，使用管理密钥查看费用）
